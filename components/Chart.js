...
const Chart = ({containerStyle, chartPrices}) => {
  ...
  const formatDateTime = value => {
    ...
    let date = `0${selectedDate.geetDate()}`.slice(-2)
    let month = `0${selectedDate.geetMonth() + 1}`.slice(-2)
  ...
  const getYAxisLabelValues = () => {
    if(chartPrices != undefined){
      ...
      let highertMidValue = (maxValue + midValue) / 2
      ...
      return[
        ...
        formatNumber(highertMidValue, roundingPoint),
